            ...
            <div style="display: grid; grid-template-columns: 1fr; gap: 20px;">
                @foreach ($ownedTeams as $team)
                    <div style="background: rgba(255, 255, 255, 0.95); border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.2); border-left: 4px solid #17a2b8;">
                        ...
                        <!-- Card Actions -->
                        <div style="padding: 0 20px 20px 20px; display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px;">
                            <a href="/teams/{{ $team->id }}" class="btn btn-done" style="padding: 10px; text-decoration: none; text-align: center; font-weight: 600; border: none; cursor: pointer; font-size: 13px;">Manage</a>
                            <a href="/teams/{{ $team->id }}/edit" class="btn" style="background: #f59e0b; padding: 10px; text-decoration: none; text-align: center; font-weight: 600; border: none; cursor: pointer; font-size: 13px;">Edit</a>
                            <form action="/teams/{{ $team->id }}" method="POST" style="display: block;">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-delete" onclick="openModal(this)" style="width: 100%; padding: 10px; border: none; cursor: pointer; font-size: 13px;">Delete</button>
                            </form>
                        </div>
                    </div>
                @endforeach
                    ...
                    <div style="background: rgba(255, 255, 255, 0.95); border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.2); border-left: 4px solid #8b5cf6;">
                        ...
                        <div style="background: linear-gradient(135deg, rgba(139, 92, 246, 0.1) 0%, rgba(107, 33, 168, 0.1) 100%); padding: 20px;">
                            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px; flex-wrap: wrap;">
                                <h3 style="margin: 0; color: #333; font-size: 18px; font-weight: 700; word-break: break-word;">{{ $team->name }}</h3>
                                <span style="background: #dbeafe; color: #1e40af; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; white-space: nowrap;">Member</span>
                            </div>
