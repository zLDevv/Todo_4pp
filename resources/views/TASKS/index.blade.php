...
<div class="container">
    ...
    <div style="display: flex; gap: 8px; margin-bottom: 20px; flex-wrap: wrap;">
        <a href="/tasks" class="btn" style="background: {{ request('filter') == null && request('search') == null ? '#17a2b8' : '#6b7280' }}; padding: 8px 14px; text-decoration: none; font-size: 13px; border-radius: 6px;">All</a>
        <a href="/tasks?filter=pending" class="btn" style="background: {{ request('filter') == 'pending' ? '#f59e0b' : '#6b7280' }}; padding: 8px 14px; text-decoration: none; font-size: 13px; border-radius: 6px;">⏳ Pending</a>
        <a href="/tasks?filter=done" class="btn" style="background: {{ request('filter') == 'done' ? '#10b981' : '#6b7280' }}; padding: 8px 14px; text-decoration: none; font-size: 13px; border-radius: 6px;">✅ Done</a>
    </div>

    <!-- Add Task Button -->
    <a href="/tasks/create" class="btn btn-add" style="width: 100%; padding: 14px; text-align: center; text-decoration: none; font-weight: 600; border: none; cursor: pointer;">+ Add New Task</a>
    ...
    <form method="GET" style="margin-bottom: 25px; display: flex; gap: 8px; flex-direction: column; align-items: stretch;">
        <div style="flex: 1; display: flex; align-items: center; background: white; border-radius: 8px; border: 2px solid #e0e0e0; transition: all 0.3s ease; width: 100%;">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search tasks..." style="flex: 1; border: none; padding: 12px 0; padding-right: 12px; font-size: 16px; width: 100%;">
        </div>
        ...
        <div class="card" style="margin-bottom: 18px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 8px; margin-bottom: 12px;">
                <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 8px; flex-wrap: wrap;">
                    <h3 class="{{ $task->status == 'done' ? 'done' : '' }}" style="margin: 0; font-size: 16px; flex: 1; min-width: 150px;">
                        {{ $task->title }}
                    </h3>
            ...
            <!-- Task Info -->
            <div style="display: grid; grid-template-columns: 1fr; gap: 8px; margin: 12px 0; padding: 12px 0; border-top: 1px solid rgb(255, 255, 255); border-bottom: 1px solid rgb(255, 255, 255); font-size: 13px;">
                <div>
                    <span style="color: rgb(255, 255, 255);">Deadline:</span>
                    <p style="margin: 4px 0 0 0; color: {{ strtotime($task->deadline) < time() ? '#ef4444' : 'white' }}; font-weight: 500;">
                        {{ date('M d, Y', strtotime($task->deadline)) }}
                    </p>
                </div>
                @if(strtotime($task->deadline) - time() < 432000 && $task->status != 'done')
                    <div style="color: #ef4444; font-weight: 600;">⚠️ Deadline Approaching!!!</div>
                @endif
            </div>

            <!-- Task Actions -->
            <div style="display: flex; gap: 6px; flex-direction: column; justify-content: space-between; align-items: stretch;">
                <div style="display: flex; gap: 6px; flex-wrap: wrap; width: 100%;">
                    <form action="/tasks/{{ $task->id }}/favorite" method="POST" style="display: inline; flex: 1; min-width: 100px;">
                        @csrf
                        <button class="btn" style="background: {{ $task->favorite ? '#fbbf24' : '#6b7280' }}; padding: 8px 10px; font-size: 11px; border: none; cursor: pointer; width: 100%;">{{ $task->favorite ? '⭐ Fav' : '☆ Fav' }}</button>
                    </form>

                    @if ($task->status == 'pending')
                        <form action="/tasks/{{ $task->id }}/done" method="POST" style="display: inline; flex: 1; min-width: 100px;">
                            @csrf
                            <button class="btn btn-done" style="padding: 8px 10px; font-size: 11px; border: none; cursor: pointer; width: 100%;">✓ Done</button>
                        </form>
                    @else
                        <form action="/tasks/{{ $task->id }}/undone" method="POST" style="display: inline; flex: 1; min-width: 100px;">
                            @csrf
                            <button class="btn" style="background: #8b5cf6; padding: 8px 10px; font-size: 11px; border: none; cursor: pointer; width: 100%;">↩️ Undo</button>
                        </form>
                    @endif

                    <a href="/tasks/{{ $task->id }}/edit" class="btn" style="background: #f59e0b; padding: 8px 10px; font-size: 11px; text-decoration: none; border: none; cursor: pointer; flex: 1; min-width: 80px; text-align: center;">✏️ Edit</a>
                </div>

                <div style="display: flex; gap: 6px;">
                    <form action="/tasks/{{ $task->id }}" method="POST" style="display: inline; width: 100%;">
                        @csrf
                        @method('DELETE')
                        <button type="button" class="btn btn-delete" onclick="openModal(this)" style="width: 100%; padding: 8px 10px; font-size: 11px;">🗑️ Delete</button>
                    </form>
                </div>
            </div>
        </div>
